...
LIVEAPP.controller('loginCtrl', function($scope, $interval, storageService, toastr, websocketService) {
    ...
    me.doLogin = function(){
        ...
        if(me.account && me.password) {
            ...
            websocketService.request('036001', submitData, function(command) {
                if(command.success) {
                    storageService.set('__account_agent', submitData.username);
                    storageService.set('__rememberMe_agent', (me.rememberMe ? '1' : '0'));
    ...
    function initPage(){
        var account = storageService.get('__account_agent');
        if(account) {
            me.account = account;
            me.hasAccount = true;
        }
        me.rememberMe = storageService.get('__rememberMe_agent') == '1';
    };
    initPage();
});
</@compress>