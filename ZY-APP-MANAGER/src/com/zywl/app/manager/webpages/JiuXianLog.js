...
if(!window.CtrlFn_${menuId}){
    window.CtrlFn_${menuId} = function($scope, $rootScope, $filter, $interval,$timeout, $compile, $templateCache, datatableUtil, alertUtil, amountUtil, storageService, websocketService, toastr){
        ...
        function initTable() {
            baseTable = datatableUtil.init('#${menuId}_baseTable', {
                ...
                columns: [
                    ...
                    {data: "roomId", title: '房间ID', render: datatableUtil.renderNormal},
                    ...
                    {data: "cardInfo", title: '酒量信息', render: datatableUtil.renderNormal},
                    {data: "winLose", title: '结果', render: function(data, type, row, setting){
                            return parseInt(data) == 1 ? "胜利" : "失败";
                        }},
                    {data: "winAmount", title: '收益', render: datatableUtil.renderNormal},
                    {data: "winValue", title: '胜利酒量', render: datatableUtil.renderNormal},
                    {data: "winInfo", title: '胜利玩家ID', render: datatableUtil.renderNormal},
                    {data: "recordTime", title: '创建时间', render: datatableUtil.renderTime},
                    ],
        ...
        function loadData(data, callback, settings){
            ...
            websocketService.request('021131', searchData, function(command){
                if(command.success){
