...
if(!window.CtrlFn_${menuId}){
    window.CtrlFn_${menuId} = function($scope, $filter, $interval,$timeout, $compile, $templateCache, datatableUtil, alertUtil, amountUtil, storageService, websocketService, toastr){
        ...
        var playerStatusDic = {'1': '正常', '4': '禁用', '5': '重新审核'};
        var playerStatusColorDic = {'1': '', '4': 'text-gray', '5': 'text-primary'};
        var playerStatusEditDic = {'1': '正常', '4': '禁用', '5': '重新审核'};
        ...
        function initTalbe() {
            baseTable = datatableUtil.init('#${menuId}_baseTable', {
                ...
                columns: [
                    ...
                    {data: "phone", title: '用户名', render: datatableUtil.renderNormal},
                    ...
                    {data: "playerLiveType", title: '频道', render: function(data, type, row, setting){
                        return data == null ? '-' :'<span class="' + playerLiveTypeColorDic[data] + '">' + playerLiveTypeDic[data] + '</span>';
                    }},
                    {data: "familyName", title: '所属家族', render: datatableUtil.renderNormal},
                    ...
                    {data: "registIpTxt", title: '注册IP归属地', render: function(data, type, row, setting){
                        return data ? '<a href="javascript:;" data-toggle="tooltip" data-placement="top" data-trigger="hover" title="' + row.registIp + '">' + data + '</a>' : '-';
                    }},
                    ...
                    {data: "lastLoginIpTxt", title: '最后登录IP归属地', render: function(data, type, row, setting){
                        return data ? '<a href="javascript:;" data-toggle="tooltip" data-placement="top" data-trigger="hover" title="' + row.lastLoginIp + '">' + data + '</a>' : '-';
                    }},
                    ...
                    {data: "id", title: '操作', width: '220px', render: function(data, type, row, setting) {
                        var _opearHtml = '<button class="btn waves-effect waves-light btn-primary btn-sm mr-2" ng-click="${menuId}.updateRow(\'' + row.id + '\')"><i class="ti-slice"></i>修改</button>'
                            + '<button class="btn waves-effect waves-light btn-info btn-sm mr-2" ng-click="${menuId}.timeRow(\'' + row.id + '\')"><i class="fa fa-clock-o"></i>统计开播时长</button>'
                            ...
                            + '<button class="btn waves-effect waves-light btn-inverse btn-sm mr-2" ng-click="${menuId}.logRow(\'' + row.id + '\')"><i class="ti-notepad"></i>资产</button>'
                            + '<button class="btn waves-effect waves-light btn-info btn-sm mr-2" ng-class="{\'d-none\': ${menuId}.hideLocation}" ng-click="${menuId}.mapRow(\'' + row.id + '\')"><i class="ti-map-alt"></i>定位</button>';
                            /*
                            + '<button class="btn waves-effect waves-light btn-danger btn-sm mr-2" ng-click="${menuId}.amountRow(\'' + row.id + '\')"><i class="ti-money"></i>金币校正</button>'
                            */
                        return _opearHtml;
        ...
        function loadData(data, callback, settings) {
            ...
            $('#${menuId}_search').find('input[name],select[name]').each(function(i, r) {
                ...
                } else if(_name == 'playerStatus') {
                    if(_value && _value.length > 0) {
                        _value = _value.join(',');
                    } else {
                        _value = Object.keys(playerStatusDic).join(',');
                    }
                }
            ...
            me.loading = true;
            websocketService.request('010004', searchData, function(command){
                if(command.success){
        ...
        /*统计开播时长*/
        me.timeRow = function(id){
            var record = dataMap[id];
            if(record) {
                me.currentTpl = record;
                me.currentTpl._title = record.phone + ' - ' + (record.nickname || '');
                me.currentTpl._errorMsg = null;
                me.currentTpl.$type = 'time';
                $('#${menuId}_formTime').find('input[name],select[name],textarea[name]').val('').trigger('change');
                $('#${menuId}_timeTotal').html('');
                $('#${menuId}_timeModal').modal('show');
            }
        };
        /*返回至列表*/
        ...
        me.saveData = function() {
            ...
            $('#${menuId}_formEdit').find('input[name],select[name],textarea[name]').each(function(i, r) {
                var _name = $(r).attr('name');
                var _value = $(r).val() || null;
                if(_name == 'playerGiftFee' || _name == 'playerTicketFee' || _name == 'playerChargeFee' || _name == 'playerGuardFee'){
            ...
            if(!testFee(submitData.playerGiftFee) || !testFee(submitData.playerTicketFee) || !testFee(submitData.playerChargeFee) || !testFee(submitData.playerGuardFee)) {
                toastr.warning('分成比例，0.1 ~ 1之间', '温馨提示');
                return false;
            }
        ...
        /*统计时长*/
        me.timeData = function() {
            if(me.loading) return;
            var submitData = {
                userId: me.currentTpl.id
            };
            $('#${menuId}_formTime').find('input[name],select[name],textarea[name]').each(function(i, r) {
                var _name = $(r).attr('name');
                var _value = $(r).val() || null;
                submitData[_name] = _value == null ? null : $.trim(_value);
            });
            if(!submitData.startDate || !submitData.endDate) {
                toastr.warning('请选择开始、结束时间', '温馨提示');
                return false;
            }
            me.loading = true;
            websocketService.request('031001', submitData, function(command){
                var strHtml = '-';
                if(command.success){
                    var result = +(command.data);
                    var _timeMinute = parseInt(result / 60);
                    if(_timeMinute > 60) {
                        strHtml = parseInt(_timeMinute/ 60) + ' 小时 ' + (_timeMinute%60) + ' 分钟';
                    } else {
                        strHtml = _timeMinute + '分钟';
                    }
                } else {
                    me.currentTpl._errorMsg = command.message || '';
                    toastr.error(command.message || '提交异常', '系统提示');
                }
                $('#${menuId}_timeTotal').html(strHtml);
                me.loading = false;
                $scope.$apply();
            });
        };
        /*本周*/
        me.localWeek = function(noSearch){
            $('#${menuId}_content input[id$="-time-player"]').val('').datepicker('update');
            var sDate = new Date();
            var eDate = new Date();
            sDate.setDate(eDate.getDate() - ((eDate.getDay() || 7) -1));
            $('#${menuId}_content input[id$="-start-time-player"]').datepicker('setDate', datatableUtil.formatDate(sDate, 'yyyy-MM-dd'));
            $('#${menuId}_content input[id$="-end-time-player"]').datepicker('setDate', datatableUtil.formatDate(eDate, 'yyyy-MM-dd'));
            if(!noSearch) {
                me.timeData();
            }
        };
        /*本月*/
        me.localMonth = function(noSearch){
            $('#${menuId}_content input[id$="-time-player"]').val('').datepicker('update');
            var sDate = new Date();
            var eDate = new Date();
            sDate.setDate(1);
