...
if(!window.CtrlFn_${menuId}){
    window.CtrlFn_${menuId} = function($scope, $rootScope, $filter, $interval,$timeout, $compile, $templateCache, $stateParams, datatableUtil, alertUtil, amountUtil, storageService, websocketService, toastr){
        ...
        function initTable() {
            baseTable = datatableUtil.init('#${menuId}_baseTable', {
                ...
                columns: [
                    ...
                    {data: "capitalType", title: '资产类型', render: function(data, type, row, setting){
                            return capitalType[data] || '-';
                        }},
                    {data: "balance", title: '余额', render: datatableUtil.renderMoney},
                    {data: "occupyBalance", title: '占用金额', render: datatableUtil.renderMoney},
        ...
        function loadData(data, callback, settings){
            ...
            websocketService.request('021050', searchData, function(command){
                if(command.success){
