...
if(!window.CtrlFn_${menuId}){
    window.CtrlFn_${menuId} = function($scope, $rootScope, $filter, $interval,$timeout, $compile, $templateCache, $stateParams, datatableUtil, alertUtil, amountUtil, storageService, websocketService, toastr){
        ...
        function initTable() {
            baseTable = datatableUtil.init('#${menuId}_baseTable', {
                ...
                columns: [
                    ...
                    {data: "petId", title: '灵兽ID', render: datatableUtil.renderNormal},
                    {data: "pl", title: '疲劳', render: datatableUtil.renderNormal},
                    {data: "nextPlTime", title: '下次疲劳恢复时间', render: datatableUtil.renderNormal},
                    {data: "lastStolenTime", title: '上次偷蛋时间', render: datatableUtil.renderNormal},
                    {data: "hasBuff", title: '是否有BUFF', render: datatableUtil.renderNormal},
                    {data: "buffId", title: 'BUFF-ID', render: datatableUtil.renderNormal},
                    {data: "receive", title: '已领取龙蛋', render: function(data, type, row, setting){
                            if(data == null || data == "") {
                                return "-";
                            }
                            let jstr = JSON.parse(data);
                            return jstr.number;
                        }},
                    {data: "unReceive", title: '未领取龙蛋', render: function(data, type, row, setting){
                            if(data == null || data == "") {
                                return "-";
                            }
                            let jstr = JSON.parse(data);
                    ...
                    {data: "outputTime", title: '产出时间', render: function(data, type, row, setting){
                            return data + " 秒";
                        }},
                    {data: "dieTime", title: '死亡时间', render: datatableUtil.renderTime},
        ...
        function loadData(data, callback, settings){
            ...
            websocketService.request('021060', searchData, function(command){
                if(command.success){
