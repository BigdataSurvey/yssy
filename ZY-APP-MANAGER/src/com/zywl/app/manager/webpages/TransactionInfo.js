...
if(!window.CtrlFn_${menuId}){
    window.CtrlFn_${menuId} = function($scope, $rootScope, $filter, $interval,$timeout, $compile, $templateCache, $stateParams, datatableUtil, alertUtil, amountUtil, storageService, websocketService, toastr){
        ...
        function initTable() {
            baseTable = datatableUtil.init('#${menuId}_baseTable', {
                ...
                columns: [
                    ...
                    {data: "itemName", title: '道具名称', render: datatableUtil.renderNormal},
                    /*{data: "itemType", title: '道具类型', render: datatableUtil.renderNormal},*/
                    {data: "itemNumber", title: '道具数量', render: datatableUtil.renderNormal},
                    {data: "itemAllNumber", title: '道具总量', render: datatableUtil.renderNormal},
                    {data: "itemPrice", title: '单价', render: datatableUtil.renderNormal},
                    {data: "type", title: '类型', render: function(data, type, row, setting){
                        let str = parseInt(data) == 0 ? "出售" : "求购";
                        return str;
                    }},
        ...
        function loadData(data, callback, settings){
            ...
            websocketService.request('021080', searchData, function(command){
                if(command.success){
