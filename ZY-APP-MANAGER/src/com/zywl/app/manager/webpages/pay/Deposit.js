...
$(function(){
    ...
    var channelMap = {
        ...
        'HigOuSJ17w9Ze3Ya86cKmoiGT2ekOq39': {
            choose: false, mark: '大额推荐，最小充值2050', order: 50
        },
        'AHWmvgUveBrj0hpWkdsZDEBV4gpofgAq': {
            choose: false, mark: '大额推荐，最小充值2050', order: 40
        },
        'ssVFOCWglFhgo0oZvjOYL1ZBFT5UPzKd': {
            choose: true, list:[54,86,107,214,535,946], mark: '方便、快捷'
        },
        'UFR3FlmlUQO3ubtB8LFSJLvRC2O21qHm': {
            choose: true, list:[54,86,107,214,535,946], mark: '方便、快捷'
        },
        'vBQKFpm0il1qYq8O3CqvSsmLhqY14ucT': {
            choose: true, list: [200, 500, 1000, 2000, 3000, 5000], mark: '金额200-5000'
        },
        'hbWh5nrGkLJAaJjMq7dgfVf2PR2dHTW5': {
            choose: true, list: [196, 296, 387, 485, 1012, 1515, 1996, 2505, 2993], mark: '方便、快捷', order: 10
        },
        'onyL2PZ99F52V4sNPODGsssPUuJuuyPn': {
            choose: true, list: [196, 296, 387, 485, 1012, 1515, 1996, 2505, 2993], mark: '方便、快捷', order: 20
        },
        't2mZngJNjz5XrdNaU6fzs7XBjCBvEz2j': {
            choose: false, mark: '大额推荐，最小充值1000', order: 60, test: true
        }
    };
           ...
           if(command.success) {
               ...
               if(_hasChange || window._test) {
                   ...
                   for(var i = 0; i < dataList.length; i++) {
                       var _r = dataList[i];
                       var _c = channelMap[_r.id];
                       var _order = _r.order || 0;
                       if(_c && _c.order > 0) {
                           _order = _c.order;
                       }
                       _r.order = _order;
                   }
                   dataList.sort(compareAsc('order'));
    ...
    function renderChannel(dataList) {
        ...
        $('#payChannel .btn-channel').on('click', function(){
            ...
            if(cRecord && !cRecord.choose) {
                ...
                if(_payChannel.hasOwnProperty('minPay') && _payChannel.minPay > 0) {
                    $('#inputAmount').attr('placeholder', '最小金额 ' + parseInt(_payChannel.minPay));
                }
                $('#rateAmount').addClass('d-none');
            } else {
    ...
    function renderAmountList() {
        ...
        if(cRecord && cRecord.choose) {
            ...
            for(var i = 0; i < _amountList.length; i++) {
                ...
                strHtml += '<div class="col-4"><a href="javascript:;" class="amount-item btn-amount ' + _active + '" data-amount="' + _amount + '"><div class="money">' + _amount + '</div><div class="gold">' + parseInt(_amount * currentTpl.rate) + '</div></a></div>';
            }
            $('#payInput').addClass('d-none');
            ...
            var _payChannel = payChannelMap[currentTpl.channelId];
            if(_payChannel && _payChannel.hasOwnProperty('minPay') && _payChannel.minPay > 0) {
                $('#inputAmount').attr('placeholder', '最小金额 ' + parseInt(_payChannel.minPay));
            }
            $('#rateAmount').addClass('d-none');
        }
