...
//        }
     ...
     */
    @Transactional
    public void subUserBalanceBySeedExchange(Long userId,BigDecimal amount, Integer capitalType,String orderNo,Long sourceDataId,LogCapitalTypeEnum logType) {
        if (userId == null || userId <= 0) {
            throwExp("userId不能为空");
        }
        if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
        ...
        // 默认日志类型为send_mail
        if (logType == null) {
            logType = LogCapitalTypeEnum.seed_exchange;
        }
        // 从缓存获取用户资产
        UserCapital userCapital = userCapitalCacheService.getUserCapitalCacheByType(userId, capitalType);
        if (userCapital == null) {
