...
@ServerEndpoint(value = "/PbxServer"
		+ SocketConstants.SOCKET_CONNECT_SHAKE_HANDS, configurator = HttpSessionConfigurator.class)
public class PbxSocketServer extends BaseServerSocket {
	private static final Log logger = LogFactory.getLog(PbxSocketServer.class);
	...
	public PbxSocketServer() {
		...
		lhdService = SpringUtil.getService(PbxService.class);
	...
	private void initPush() {
		...
		Push.registPush(new PushBean(PushCode.updateGameStatus), new PushListener() {
			...
			public void onReceive(BaseSocket baseSocket, Object data) {
				...
				if (gameId == 10) {
					PbxService.STATUS = json.getIntValue("status");
				}
			}
		}, this);
